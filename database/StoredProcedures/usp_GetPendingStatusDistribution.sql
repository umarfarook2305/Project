CREATE PROCEDURE [dbo].[usp_GetPendingStatusDistribution]
...
BEGIN
	...
	BEGIN TRY
		...
		WITH PendingRoleCounts AS (
			SELECT 
				a.RequestToRole,
				COUNT(DISTINCT r.RequestId) AS Count
			FROM dbo.Approval a
			INNER JOIN dbo.Request r ON a.RequestId = r.RequestId
			INNER JOIN dbo.PositionStatus ps ON a.PositionStatusId = ps.PositionStatusId
			WHERE ps.StatusName = 'Pending'
				AND r.RequestedOn >= @CalculatedStartDate
				AND r.RequestedOn <= @CalculatedEndDate
				AND a.RequestToRole IS NOT NULL
				AND (
					@InsightsType = 'All'
			...
			GROUP BY a.RequestToRole
		)
		SELECT 
			'200' AS StatusCode,
			'success' AS Status,
			prc.RequestToRole AS RequesterRole,
			prc.Count,
			@CalculatedStartDate AS StartDate,
			@CalculatedEndDate AS EndDate
		FROM PendingRoleCounts prc
		ORDER BY prc.RequestToRole;
